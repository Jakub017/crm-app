@extends('notes.layouts.main')
@section('title', 'Notatki')

@section('content')

<div class="notes">
    <h2 class="notes__title">Twoje notatki</h2>
    <div class="notes__wrapper">
        <div class="notes__note">
            <div class="notes__text">
                <h2 class="notes__name">Tytuł notatki</h2>
                <p class="notes__content">Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>
            </div>
            <div class="notes__info">
                <span class="notes__date">7 lutego 2023</span>
                <div class="notes__actions">
                    <a href="#" class="notes__edit"><svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M21.875 8.71481L17.6 4.67037L18.675 3.63518C19.075 3.25 19.5543 3.0574 20.113 3.0574C20.671 3.0574 21.1417 3.25 21.525 3.63518L22.925 5.0074C23.3083 5.37654 23.5 5.82592 23.5 6.35555C23.5 6.88518 23.3083 7.33456 22.925 7.7037L21.875 8.71481ZM5 22.8222C4.45 22.8222 3.979 22.6335 3.587 22.256C3.19567 21.8792 3 21.4259 3 20.8963V7.41481C3 6.88518 3.19567 6.43163 3.587 6.05414C3.979 5.6773 4.45 5.48888 5 5.48888H13.925L7.875 11.3148C7.59167 11.5877 7.375 11.9006 7.225 12.2537C7.075 12.6068 7 12.9759 7 13.3611V17.0444C7 17.5741 7.196 18.0273 7.588 18.4041C7.97933 18.7816 8.45 18.9704 9 18.9704H12.825C13.225 18.9704 13.6083 18.8981 13.975 18.7537C14.3417 18.6093 14.6667 18.4006 14.95 18.1278L21 12.2778V20.8963C21 21.4259 20.8043 21.8792 20.413 22.256C20.021 22.6335 19.55 22.8222 19 22.8222H5ZM10 17.0444C9.71667 17.0444 9.47933 16.9523 9.288 16.7681C9.096 16.5832 9 16.3543 9 16.0815V13.7463C9 13.4895 9.05 13.2446 9.15 13.0116C9.25 12.7792 9.39167 12.5747 9.575 12.3981L16.175 6.04259L20.45 10.087L13.825 16.4667C13.6417 16.6432 13.4293 16.7838 13.188 16.8884C12.946 16.9924 12.6917 17.0444 12.425 17.0444H10Z"
                                fill="#848484" fill-opacity="0.7" />
                        </svg>
                    </a>
                    <a href="#" class="notes__delete"><svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M19 3.84615H15.5L14.5 2.88461H9.5L8.5 3.84615H5V5.76923H19M6 18.2692C6 18.7793 6.21071 19.2684 6.58579 19.6291C6.96086 19.9897 7.46957 20.1923 8 20.1923H16C16.5304 20.1923 17.0391 19.9897 17.4142 19.6291C17.7893 19.2684 18 18.7793 18 18.2692V6.73077H6V18.2692Z"
                                fill="#848484" fill-opacity="0.7" />
                        </svg>
                    </a>
                </div>
            </div>
        </div>
        <div class="notes__note">
            <div class="notes__text">
                <h2 class="notes__name">Tytuł notatki</h2>
                <p class="notes__content">Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>
            </div>
            <div class="notes__info">
                <span class="notes__date">7 lutego 2023</span>
                <div class="notes__actions">
                    <a href="#" class="notes__edit"><svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M21.875 8.71481L17.6 4.67037L18.675 3.63518C19.075 3.25 19.5543 3.0574 20.113 3.0574C20.671 3.0574 21.1417 3.25 21.525 3.63518L22.925 5.0074C23.3083 5.37654 23.5 5.82592 23.5 6.35555C23.5 6.88518 23.3083 7.33456 22.925 7.7037L21.875 8.71481ZM5 22.8222C4.45 22.8222 3.979 22.6335 3.587 22.256C3.19567 21.8792 3 21.4259 3 20.8963V7.41481C3 6.88518 3.19567 6.43163 3.587 6.05414C3.979 5.6773 4.45 5.48888 5 5.48888H13.925L7.875 11.3148C7.59167 11.5877 7.375 11.9006 7.225 12.2537C7.075 12.6068 7 12.9759 7 13.3611V17.0444C7 17.5741 7.196 18.0273 7.588 18.4041C7.97933 18.7816 8.45 18.9704 9 18.9704H12.825C13.225 18.9704 13.6083 18.8981 13.975 18.7537C14.3417 18.6093 14.6667 18.4006 14.95 18.1278L21 12.2778V20.8963C21 21.4259 20.8043 21.8792 20.413 22.256C20.021 22.6335 19.55 22.8222 19 22.8222H5ZM10 17.0444C9.71667 17.0444 9.47933 16.9523 9.288 16.7681C9.096 16.5832 9 16.3543 9 16.0815V13.7463C9 13.4895 9.05 13.2446 9.15 13.0116C9.25 12.7792 9.39167 12.5747 9.575 12.3981L16.175 6.04259L20.45 10.087L13.825 16.4667C13.6417 16.6432 13.4293 16.7838 13.188 16.8884C12.946 16.9924 12.6917 17.0444 12.425 17.0444H10Z"
                                fill="#848484" fill-opacity="0.7" />
                        </svg>
                    </a>
                    <a href="#" class="notes__delete"><svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M19 3.84615H15.5L14.5 2.88461H9.5L8.5 3.84615H5V5.76923H19M6 18.2692C6 18.7793 6.21071 19.2684 6.58579 19.6291C6.96086 19.9897 7.46957 20.1923 8 20.1923H16C16.5304 20.1923 17.0391 19.9897 17.4142 19.6291C17.7893 19.2684 18 18.7793 18 18.2692V6.73077H6V18.2692Z"
                                fill="#848484" fill-opacity="0.7" />
                        </svg>
                    </a>
                </div>
            </div>
        </div>
        <div class="notes__note">
            <div class="notes__text">
                <h2 class="notes__name">Tytuł notatki</h2>
                <p class="notes__content">Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>
            </div>
            <div class="notes__info">
                <span class="notes__date">7 lutego 2023</span>
                <div class="notes__actions">
                    <a href="#" class="notes__edit"><svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M21.875 8.71481L17.6 4.67037L18.675 3.63518C19.075 3.25 19.5543 3.0574 20.113 3.0574C20.671 3.0574 21.1417 3.25 21.525 3.63518L22.925 5.0074C23.3083 5.37654 23.5 5.82592 23.5 6.35555C23.5 6.88518 23.3083 7.33456 22.925 7.7037L21.875 8.71481ZM5 22.8222C4.45 22.8222 3.979 22.6335 3.587 22.256C3.19567 21.8792 3 21.4259 3 20.8963V7.41481C3 6.88518 3.19567 6.43163 3.587 6.05414C3.979 5.6773 4.45 5.48888 5 5.48888H13.925L7.875 11.3148C7.59167 11.5877 7.375 11.9006 7.225 12.2537C7.075 12.6068 7 12.9759 7 13.3611V17.0444C7 17.5741 7.196 18.0273 7.588 18.4041C7.97933 18.7816 8.45 18.9704 9 18.9704H12.825C13.225 18.9704 13.6083 18.8981 13.975 18.7537C14.3417 18.6093 14.6667 18.4006 14.95 18.1278L21 12.2778V20.8963C21 21.4259 20.8043 21.8792 20.413 22.256C20.021 22.6335 19.55 22.8222 19 22.8222H5ZM10 17.0444C9.71667 17.0444 9.47933 16.9523 9.288 16.7681C9.096 16.5832 9 16.3543 9 16.0815V13.7463C9 13.4895 9.05 13.2446 9.15 13.0116C9.25 12.7792 9.39167 12.5747 9.575 12.3981L16.175 6.04259L20.45 10.087L13.825 16.4667C13.6417 16.6432 13.4293 16.7838 13.188 16.8884C12.946 16.9924 12.6917 17.0444 12.425 17.0444H10Z"
                                fill="#848484" fill-opacity="0.7" />
                        </svg>
                    </a>
                    <a href="#" class="notes__delete"><svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M19 3.84615H15.5L14.5 2.88461H9.5L8.5 3.84615H5V5.76923H19M6 18.2692C6 18.7793 6.21071 19.2684 6.58579 19.6291C6.96086 19.9897 7.46957 20.1923 8 20.1923H16C16.5304 20.1923 17.0391 19.9897 17.4142 19.6291C17.7893 19.2684 18 18.7793 18 18.2692V6.73077H6V18.2692Z"
                                fill="#848484" fill-opacity="0.7" />
                        </svg>
                    </a>
                </div>
            </div>
        </div>
        <div class="notes__note">
            <div class="notes__text">
                <h2 class="notes__name">Tytuł notatki</h2>
                <p class="notes__content">Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>
            </div>
            <div class="notes__info">
                <span class="notes__date">7 lutego 2023</span>
                <div class="notes__actions">
                    <a href="#" class="notes__edit"><svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M21.875 8.71481L17.6 4.67037L18.675 3.63518C19.075 3.25 19.5543 3.0574 20.113 3.0574C20.671 3.0574 21.1417 3.25 21.525 3.63518L22.925 5.0074C23.3083 5.37654 23.5 5.82592 23.5 6.35555C23.5 6.88518 23.3083 7.33456 22.925 7.7037L21.875 8.71481ZM5 22.8222C4.45 22.8222 3.979 22.6335 3.587 22.256C3.19567 21.8792 3 21.4259 3 20.8963V7.41481C3 6.88518 3.19567 6.43163 3.587 6.05414C3.979 5.6773 4.45 5.48888 5 5.48888H13.925L7.875 11.3148C7.59167 11.5877 7.375 11.9006 7.225 12.2537C7.075 12.6068 7 12.9759 7 13.3611V17.0444C7 17.5741 7.196 18.0273 7.588 18.4041C7.97933 18.7816 8.45 18.9704 9 18.9704H12.825C13.225 18.9704 13.6083 18.8981 13.975 18.7537C14.3417 18.6093 14.6667 18.4006 14.95 18.1278L21 12.2778V20.8963C21 21.4259 20.8043 21.8792 20.413 22.256C20.021 22.6335 19.55 22.8222 19 22.8222H5ZM10 17.0444C9.71667 17.0444 9.47933 16.9523 9.288 16.7681C9.096 16.5832 9 16.3543 9 16.0815V13.7463C9 13.4895 9.05 13.2446 9.15 13.0116C9.25 12.7792 9.39167 12.5747 9.575 12.3981L16.175 6.04259L20.45 10.087L13.825 16.4667C13.6417 16.6432 13.4293 16.7838 13.188 16.8884C12.946 16.9924 12.6917 17.0444 12.425 17.0444H10Z"
                                fill="#848484" fill-opacity="0.7" />
                        </svg>
                    </a>
                    <a href="#" class="notes__delete"><svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M19 3.84615H15.5L14.5 2.88461H9.5L8.5 3.84615H5V5.76923H19M6 18.2692C6 18.7793 6.21071 19.2684 6.58579 19.6291C6.96086 19.9897 7.46957 20.1923 8 20.1923H16C16.5304 20.1923 17.0391 19.9897 17.4142 19.6291C17.7893 19.2684 18 18.7793 18 18.2692V6.73077H6V18.2692Z"
                                fill="#848484" fill-opacity="0.7" />
                        </svg>
                    </a>
                </div>
            </div>
        </div>
    </div>
    <a href="#" class="notes__add">
        <svg class="notes__add-icon" width="40" height="40" viewBox="0 0 40 40" fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <path
                d="M30.0002 21.6633H21.6668V29.9967C21.6668 30.4387 21.4912 30.8626 21.1787 31.1752C20.8661 31.4877 20.4422 31.6633 20.0002 31.6633C19.5581 31.6633 19.1342 31.4877 18.8217 31.1752C18.5091 30.8626 18.3335 30.4387 18.3335 29.9967V21.6633H10.0002C9.55814 21.6633 9.13421 21.4877 8.82165 21.1752C8.50909 20.8626 8.3335 20.4387 8.3335 19.9967C8.3335 19.5546 8.50909 19.1307 8.82165 18.8182C9.13421 18.5056 9.55814 18.33 10.0002 18.33H18.3335V9.99667C18.3335 9.55464 18.5091 9.13072 18.8217 8.81816C19.1342 8.5056 19.5581 8.33 20.0002 8.33C20.4422 8.33 20.8661 8.5056 21.1787 8.81816C21.4912 9.13072 21.6668 9.55464 21.6668 9.99667V18.33H30.0002C30.4422 18.33 30.8661 18.5056 31.1787 18.8182C31.4912 19.1307 31.6668 19.5546 31.6668 19.9967C31.6668 20.4387 31.4912 20.8626 31.1787 21.1752C30.8661 21.4877 30.4422 21.6633 30.0002 21.6633Z"
                fill="white" />
        </svg>

    </a>
</div>

@endsection
